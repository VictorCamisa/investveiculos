...
import { Plus, Search, Workflow, Zap, Bot, RefreshCcw } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <Card className="border-primary/20 bg-primary/5">
        <CardHeader className="pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg">Regras de Reativação com IA</CardTitle>
              <CardDescription>
                Configure regras que a IA usará para reativar leads automaticamente via WhatsApp
              </CardDescription>
            </div>
          </div>
        </CardHeader>
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Regras automáticas de follow-up para reconquistar leads
          </p>
        </div>
        ...
        <div className="text-center py-12 bg-card border rounded-lg">
          ...
          <p className="text-muted-foreground mb-4">
            {searchTerm
              ...
              : 'Crie seu primeiro fluxo de reativação'}
          </p>
          {!searchTerm && (
            <Button onClick={() => setIsFormOpen(true)} className="gap-2">
              <Plus className="h-4 w-4" />
      ...
      <Dialog open={isFormOpen} onOpenChange={handleCloseForm}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingFlow ? 'Editar Fluxo de Reativação' : 'Novo Fluxo de Reativação'}
            </DialogTitle>
          </DialogHeader>
          <FollowUpFlowForm
